...
| 模块       | 功能                                                                   |
| ---------- | ---------------------------------------------------------------------- |
| 通用有向图 | 表单/JSON 添加节点(含分组颜色)和有向边(含权重)，拖拽节点，6 种自动布局 |
| SAM 构建   | 输入 a-z 字符串，纯前端 JS 实时构建 SAM 并渲染 DAG                     |
| 三视图切换 | 转移边(实线) / 后缀链接(红色虚线) / 合并                               |
| 最长字符串 | 切换节点标签：显示 `len` 或对应最长子串                                |
| 信息面板   | 侧边栏实时展示状态表、转移边表、后缀链接表                             |
| 导出       | PNG 截图 / JSON 数据                                                   |
| 编辑       | 点击画布节点/边弹出编辑面板，支持修改标签、分组、权重、删除            |
...
│   └── components/
│       ├── InputPanel.jsx       # 节点/边表单 + SAM 输入框 + JSON 导入
│       ├── GraphCanvas.jsx      # Cytoscape 渲染 + 拖拽 + 点击编辑
│       ├── Toolbar.jsx          # 布局/导出按钮 + 视图切换 + 标签格式切换
...
- **合并**: 两种边同时显示
- **标签切换**: 点击 `[len]` / `[字符串]` 切换节点显示 `0 (len=0)` / `0 ("")`
...
3. 打开浏览器 `http://localhost:3000?sam`，Web 端自动 fetch JSON 并渲染
